...
export const ResumeUpload = ({ roleId, onResumeProcessed, minimumScore = 60, showBestMatch = false }: ResumeUploadProps) => {
  ...
  // AWS Textract disabled — using PDF.js directly (re-enable by restoring S3+Textract block in handleUpload)
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // TO RE-ENABLE: restore import of { uploadResumeToS3 } from "@/lib/resumeService"
      //   and { extractTextViaTextract } from "@/utils/atsParser",
      //   then add the S3 upload + Textract extraction block here.
